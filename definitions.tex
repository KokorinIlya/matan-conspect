...
\\
	...
	\section{Апроксимативная единица Вейерштрасса}
	Семейство функций ${w_t}_{t>0}$ - апроксимативная единица при $t \rightarrow 0$
	\begin{enumerate}
		\item $w_t \geq 0$
		\item $\int_{R^m} w_t d\lambda_m = 1$
		\item $\forall \delta > 0 \int_{|x| > \delta} w_t d\lambda_m \rightarrow 0$ при $t \rightarrow +\infty$
	\end{enumerate}
	Замечение: пункты 1 и 2 означают что $w_t \in L^1 (R^m)$
