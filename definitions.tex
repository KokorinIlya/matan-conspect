...
$(X,\mathds{A},\mu)$, $(Y,\mathds{B},\nu)$~--- пространства с мерой. $\mu,\nu$~--- $\sigma$-конечные полные меры.

$m = \mu \times \nu$ (произведение мер).

$f$~--- суммируемая функция на $X \times Y$ (по мере $m$). Обозначим: $f_x(y) = f(x, y)$, $f^y(x) = f(x, y)$.

Тогда:
\begin{enumerate}
    \item При почти всех $x$ $f_x$ суммируема.\\
          При почти всех $y$ $f^y$ суммируема.
    \item $\phi(x) = \int_Y f_x~d\nu$ суммируема.\\
          $\psi(y) = \int_X f^y~d\mu$ суммируема.
    \item $$\int_{X \times Y} f~dm = \int_X \phi(x)~d\mu = \int_X(\int_Y f(x,y) d\nu(y))d\mu(x)$$
\end{enumerate}
