...
\section{Интеграл Фурье, частичный интеграл Фурье}
	$f(x) = v.p. \lim\limits_{R \rightarrow \inf} \int\limits_{-R}^{R} \hat{f}(x) * e^{2 \pi i x y} dy$ \\
...
\section{Аппроксимативная единица Вейерштрасса. \Large Deprecated }
	Семейство функций ${w_t}_{t>0}$ - аппроксимативная единица при $t \rightarrow 0$
	\begin{enumerate}
		\item $w_t \geq 0$
		\item $\int_{R^m} w_t d\lambda_m = 1$
		\item $\forall \delta > 0 \int_{|x| > \delta} w_t d\lambda_m \rightarrow 0$ при $t \rightarrow +\infty$
	\end{enumerate}
	Замечание: пункты 1 и 2 означают что $w_t \in L^1 (R^m)$
	Замечание: $w_t$ - а.е. $f \in L^1 (R^m) \Rightarrow f * w_t \rightarrow f$ при $t \rightarrow 0$ (в $L^1(R^m)$)
