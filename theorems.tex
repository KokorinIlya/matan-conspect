...
$ S_n(h, x_0) \to 0 $\\
$ S_n(h, x_0) = \frac{1}{2\pi} \int\limits_{-\pi}^{\pi} h(x_0 + t) (ctg(\frac{t}{2}) sin(nt) + cos(nt)) dt $\\
...
В (3) : $- \int_{z_0}^{z} (A_1)'_x dz + \phi'_x - \int_{z_0}^{z} A_2'_y dz = A_3$.
...
1) $\hat{(f * g)} = \int_{R^m}(\int_{R^m} f(u)g(x-u)du)e^{-2\pi <x, y>}dx = \int_{R^m}f(u)e^{-2\pi <u, y>}(\int_{R^m} g(x-u)e^{-2\pi <x-u, y>}dx)du = \hat{f(y)}\hat{g(y)}$

2) $\int_{R^m}\hat{f(y)}g(y)dy = \int_{R^m}f(y)\hat{g(y)}dy$ (как-то аналогично доказывается)

\section{Преобразование Фурье и дифференцирование}

$f \in L^1(R^m)$
\begin{enumerate}
	\item k - номер координаты, $\frac{df}{dx_k} \in L^1, \hat{(\frac{df}{dx_k})}(y) = 2\pi i y_k f(y)$
