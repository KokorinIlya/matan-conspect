...
$f, g \in L^1(R^m)$
\begin{enumerate}
	\item $\hat{(f * g)} = \int_{R^m}(\int_{R^m} f(u)g(x-u)du)e^{-2\pi <x, y>}dx = \int_{R^m}f(u)e^{-2\pi <u, y>}(\int_{R^m} g(x-u)e^{-2\pi <x-u, y>}dx)du = \hat{f(y)}\hat{g(y)}$
	\item $\int_{R^m}\hat{f(y)}g(y)dy = \int_{R^m}f(y)\hat{g(y)}dy$ (как-то аналогично доказывается)
\end{enumerate}
