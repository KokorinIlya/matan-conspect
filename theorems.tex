...
\begin{enumerate}
	...
	\item $f(x) = inf(\alpha : x \in G_\alpha)$ и $\alpha$ - дв.рац. Нужно проверить непрерывность. То есть для любого открытого множества прообраз открыт. А для этого достаточно проверить, что $f^{-1}(-\inf, s) = \cup_{\alpha < s} G_\alpha$ и $f^{-1}(-\inf, s] = \cap_{\alpha > s} \overline{G_\alpha}$. 
	\item $\supset$. очевидно из определения f. $x \in G_\alpha \Rightarrow f(x) \leq \alpha \Rightarrow f(x) < S \Rightarrow x \in f^{-1}(-\inf, s)$
	\item $\subset$. Пусть $f(x) = s_0 < \alpha < s$. Тогда $x \in G_\alpha$ /* $f(x) = s_0 = inf(\beta : x \in G_\beta)$ Если бы $x \notin G_\alpha$, то $x \in G_\beta$ при $\beta < \alpha$ и $\inf > s_0$.
...
\emph {Тогда:} \\
   $S_n(f, x) \rightarrow_{n \rightarrow \inf} 1/2 * (f(x + 0) + f(x - 0))$ \\
...
\emph{Доказательство:}\\
	...
	$\int\limits_{0}^{\delta} \Phi(t) \frac{sin(n t)}{\pi t} dt = \int\limits_{0}^{\inf} \Phi(t) \frac{sin(n t)}{\pi t} dt = /* u=nt */ = \int\limits_{0}^{\inf} \Phi(u / n) \frac{sin(u)}{\pi u} du \rightarrow magic-knowledge \rightarrow \phi(0+) \int\limits_{0}^{\inf} \frac{sin(u)}{\pi u} du = 1 / 2 * \phi(0+)$
