...
\section{Признак Лейбница дифференцирования интеграла по параметру}
...
\section{Корректность светки}

\section{Свойства светки функции из $ L^p $ с фукнцией из $ L^q $}
...
$ | \int\limits^\pi_{-\pi} f(x - t) K(t) dt)| \underset{\text{нер-во Гельдера}}{\leq} (\int\limits^\pi_{-\pi} |f(x -t)|^p dt)^{1/p} (\int\limits^\pi_{-\pi} |K(x)|^q dt)^{1/q} = \\
 = ||K||_q ||f||_p \Rightarrow \sup|f \ast K| \leq ||f||_p ||K||_q \Rightarrow \text{пунк 2}$ 




\section{Свойства свертки}

\begin{enumerate}
	\item Свертка -- корректно заданная функция из $ L_1([-\pi, \pi]) $
	\item Коммутативность: $ f \ast K = K \ast f$
	\item $ c_k(f \ast K) = 2\pi c_k(f) c_k(K) $ ($ c_k $ -- коэф. ряда фурье)
	\item {\bf \Large deprecated}
		$ f \in L^p $; $ K \in L^q $
