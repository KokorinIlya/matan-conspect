		...
		\begin{enumerate}
			...
			\item Наглая редукция: $\forall c \in (0,1): \lim\int\limits_{X}f_n(x) \geqslant c \cdot \int\limits_{X}g$
				\begin{enumerate}
					...
					\item $\int\limits_{X}f_n \geqslant \int\limits_{E_n}f_n \geqslant \int\limits_{E_n}g$ \textcolor{red}{Это неправда, $E_n$ определялаьсь не так} $\Rightarrow \lim \int\limits_{X}f_n \geqslant c \cdot \lim \int\limits_{E_n}g = c \cdot \int\limits_{X}g$
...
\begin{enumerate}
	...
	$ \int\limits_{\mathds{X}} 2g - \int\limits_{\mathds{X}} h \Rightarrow \int\limits_{\mathds{X}} h_n \rightarrow 0 $\textcolor{red}{Что тут вообще написано?}
...
\begin{enumerate}
	...
	\item $||x||^2 = ||S_n + z||^2 \geqslant ||S_n||^2 + ||z||^2 \geqslant ||S_n||^2$
\end{enumerate}
