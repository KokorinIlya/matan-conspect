...
\section{Лемма : а.е. Веерштрасса - а.е}
\begin{enumerate}
    \item ${w_t}$ - а.е. при $t->0$
    \item $w_t(x) = \int_{R^m} e^{-\pi t^2 |y|^2} e^{-2\pi <x, y>} dy$, т.е $w_t = \^{(V_t)}$, где $V_t(x) = e^{-\pi t^2 |x|^2}$
\end{enumerate}
\emph{Доказательство:}
\begin{enumerate}
    \item $w_t \geq 0$
    ...
    $f_a(x) = e^{-\pi a^2 x^2}, x \in R, a > 0, \^{(f_a)} = \frac{1}{a} f_{\frac{1}{a}}$ (ребята TODOOOO не шарю к чему это тут ????)\\ 
    $\int_{R^m} e^{-\pi t^2 |y|^2} e^{- 2 \pi i <x, y>} dy = \prod_{k=0}^{m-1} \int_{R} e^{-\pi t^2 y_k^2} e^{- 2 \pi i <x_k, y_k>} dy_k = \prod_{k=0}^{m-1} \frac{1}{t} e^{\frac{-\pi x_k^2}{t^2}} = \frac{1}{t^m} e^{\frac{-\pi |x|^2}{t^2}}$  ( и это вообще тут зачем, помогите плиз -_- . Казалось бы это должно доказывать что $w_t * f \rightarrow f$, но я тут этого не вижу явно...)

\end{enumerate}

\section{Теорема (формула обращения)}
$f \in L^1(R^m)$
Если $\^{f} \in L^1(R^m)$, то при п.в. x $f(x) = \int_{R^m} \^{f(y)} e^{2\pi i <x, y>} dy$ \\
Замечания: \\
1) П.Ч.: подынт.ф. непр. по x \\
есть сумм. мажоранта: $|\^{f(y)}| \Rightarrow $ П.Ч. --- непрерывна по x \\
2) ПЧ. непр по x и при этом ф-ла вып. п.в. $\Rightarrow$ ф-ла верна в точке непр-сти $f$ \\
Итого: Если $f \in C(R^m), f \in L^1(R^m), \^{f} \in L^1(R^m)$, то ф-ла обращения вып. при всех $x$.
\emph{Доказательство:}
...
$= \int_{R^m} f(x+y) \^{(V_t)} dy = \int_{R^m} \^{(f(y+x))} V_t(y) dy = \int_{R^m} e^{2 \pi i <x, y>} \^{f(y)} V_t(y) dy$\\
Итак : $f * w_t(x) = \int_{R^m} e^{-pi t^2 |y|^2} e^{2 \pi i <x, y>} \^{f(y)} dy = I$ \\
...
$e^{-pi t^2 |y|^2} e^{2 \pi i <x, y>} \^{f(y)}  \rightarrow e^{2 \pi i <x, y>} \^{f(y)}$ при $t \rightarrow 0$. И есть сумм. мажоранта $|\^{f(y)}|$. Значит интеграл $I$ стремится к $\int_{R^m} e^{2 \pi i <x, y>} \^{f(y)}$\\
...
Поэтому $f(x) = \lim_{t_{n_k} \rightarrow 0} f*f*w_{t_{n_k}} = \Int \dots \rightarrow \int_{R^m} e^{2 \pi i <x, y>} \^{f(y)}$, чтд
\end{document}
